...
<div class="wrap">
    ...
    <?php if (!$this->is_api_key_valid()): ?>
        <div class="notice notice-error inline">
            <p>Could not connect to Algolia. <br>Please check <strong>Application ID</strong> and/or <strong>Admin API Key</strong>.</p>
        </div>
    <?php endif; ?>
    ...
    <div id="wlion-algolia__notice">
        <p></p>
        <button class="wlion-algolia__notice-close"></button>
    </div>
    ...
    <?php if ($this->is_api_key_valid()):?>
                ...
                <tbody>
                    <?php foreach ($indices as $k => $index): ?>
                        ...
                        <tr class="<?= $table_row_class; ?>">
                            <td class="row-title"><?= $index['name']; ?>
                            </td>
                            <td style="text-align:right;"><?= $index['entries']; ?>
                            </td>
                                ...
                                <?php if ($this->is_index_for_current_environment($index['name'])): ?>
                                    <?php if ($this->get_searchable_post_index() === $index['name']): ?>
                                        <button
                                            ...
                                            data-tab-target="searchable-posts"
                                        >
                                            Go to Searchable Posts
                                        </button>
                                    <?php else: ?>
                                        <button
                                            class="button button-primary get-index-settings"
                        ...
                        <?php if (array_key_exists('replicas_data', $index)): ?>
                            ...
                            <tr class="<?= $table_row_class; ?> replica-row">
                                <td class="replica-title"><em><?= $replica['name']; ?></em></td>
                                <td style="text-align:right;"><?= $replica['entries']; ?></td>
                                <td><?= date('Y-M-d H:i:s', strtotime($replica['updatedAt'])); ?></td>
                                <td>
                                    <?php if ($this->is_index_for_current_environment($index['name'])): ?>
                                        <button
                                            class="button button-primary get-index-settings"
                            ...
                            <tr
                                class="<?= (0 !== $k % 2) ? 'alternate ' : ''; ?>index-settings"
                                data-index="<?= $replica['name']; ?>"
                            >
                                <td colspan="5">
                                    <div class="spinner is-active"></div>
        ...
        <div data-tab="searchable-posts" class="tab-content">
            ...
            <p style="max-width: 750px;">Note: These settings will only affect <strong><em><?= $searchable_posts; ?></em></strong>. By clicking <strong>Index Searchable Posts</strong>, you are overwriting any records currently index by Algolia for this index, you can not undo this.</p>
            ...
            <div class="searchable-posts__post-types">
                <div class="searchable-posts__post-types__settings">
                    ...
                    <?php if ($this->does_index_exist($searchable_posts)):
                        ...
                        ?>
                        <form class="index-settings-form searchable-posts" data-index-form="<?= $searchable_posts ?>" >
                            ...
                            <button
                                type="submit"
                                class="button button-secondary push-settings"
                        ...
                        <?php else: ?>
                        <p class="description">(Create Searchable Posts index, and refresh the page, to load default configuration settings.)</p>
                    <?php endif; ?>
                </div>
                ...
                <div class="searchable-posts__post-types__checkboxes">
                    ...
                    <?php foreach ($this->get_all_post_types() as $post_type): ?>
                        <div class="searchable-posts__post-type__checkbox-container">
                            ...
                            />
                            <label for="<?= $post_type->name; ?>" class="searchable-posts__post-type__label">
                                <?= $post_type->labels->name; ?> (<?= $this->get_searchable_post_types_count([$post_type->name]) ?>)
                            </label>
                        </div>
                    <?php endforeach; ?>
                    <br>
                ...
                <div class="searchable-posts__post-types__count">
                    ...
                    <div class="searchable-posts__post-types__amount-container">
                        <h3 class="searchable-posts__post-types__amount">
                            <?= $this->get_searchable_post_types_count($this->settings->get_searchable_posts_data()); ?> Posts
                        </h3>
                    ...
                    <div class="searchable-posts__post-types__btn-container">
                        ...
                        <button
                            class="button button-primary index-searchable-posts"
                            data-create-settings="<?= $this->does_index_exist($searchable_posts) ? 'false' : 'true' ?>"
                        >
                            <?= $this->does_index_exist($searchable_posts) ? 'Re-Index' : 'Create' ?> Searchable Posts
                        </button>
        ...
        <div data-tab="custom-hooks" class="tab-content">
            <h2>Custom Hooks</h2>
            <p>Find these hooks in <strong><?= $this->plugin->get_custom_hooks_location(); ?></strong>.</p>
            <table class="widefat">
                <thead>
                    <tr class="alternate">
